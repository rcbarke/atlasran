...
**Generated:** 2026-02-19T00:56:54Z  
...
- **Per-UE throughput collapses** from **~101 Mbps/UE (1 UE)** to **~1.40 Mbps/UE (12 UEs)** (~**72×** drop per UE).
...
- LDPC decoding latency remains in the **~319–343 μs** range across multi-UE runs, while **LDPC events/s** drops in proportion to throughput, indicating the decoder pipeline is **not saturated**.
...
|   UEs | UL total (Mbps)   | UL/UE (Mbps)   | Jain fairness   | DU CPU mean/p95 (%)   | CU CPU mean/p95 (%)   | LDPC mean/p95 (us)   |   LDPC events/s | GPU util mean/p95 (%)   | GPU power mean/p95 (W)   |
|------:|:------------------|:---------------|:----------------|:----------------------|:----------------------|:---------------------|----------------:|:------------------------|:-------------------------|
|     0 | NA                | NA             | NA              | 208.90/211.02         | 1.92/2.20             | 208.65/222.09        |           0.571 | 1.32/2.30               | 11.83/12.23              |
|     1 | 101.20            | 101.20         | 1.000000        | 346.57/348.92         | 20.51/21.84           | 322.55/360.40        |           1.287 | 45.43/50.00             | 37.54/39.10              |
|     3 | 64.82             | 21.61          | 1.000000        | 326.69/347.02         | 16.63/18.17           | 319.22/344.05        |           0.827 | 28.33/33.90             | 28.91/30.40              |
|     6 | 32.78             | 5.46           | 0.999935        | 224.35/235.22         | 9.22/11.08            | 342.61/386.26        |           0.445 | 15.70/20.00             | 20.89/22.04              |
|    12 | 16.76             | 1.40           | 0.999995        | 175.38/182.50         | 5.98/7.16             | 337.65/377.38        |           0.245 | 8.71/13.30              | 16.58/17.55              |
...
A common failure mode in GPU-accelerated PHY pipelines is that, as offered load increases, the GPU becomes the bottleneck and we observe:
...
This points to an **I/O or coordination bottleneck** that reduces the number of PHY work items that reach the decoder (i.e., fewer TBs processed per unit time), rather than the decoder failing to keep up.
...
### 3) RFSim’s design characteristics match the observed failure mode
...
> Note: In these runs, DU logs did not show explicit `buffer overflow` events, but RFsim can still throttle via backpressure or socket-level congestion without surfacing a hard overflow error.

### 4) Why this matters: RFSim is a *loose emulator*, not a cyber-physical twin

A cyber-physical twin (digital twin) is typically expected to preserve the key timing and performance behaviors of the real system. Digital-twin definitions commonly emphasize **faithful behavior/performance** tracking—often tied to real-time or near-real-time coupling [6].
...
3. The telemetry (CPU/GPU utilization + LDPC timing) indicates the system is **not compute-limited**, and the most plausible explanation is **RFsim multi-stream contention**.
