#!/usr/bin/env python3
"""05_logs_follow_oai_nr_cu.py

Follow `docker logs -f` for the CU container and prefix each line with an
epoch timestamp for time alignment.

Example:
  python3 05_logs_follow_oai_nr_cu.py --out cu_docker_logs.tsv

TSV Output Columns:
  ts_epoch_ms  container  log_line

Notes:
- Uses docker's `--since` option so you can start from "now".
"""
