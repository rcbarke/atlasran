#!/usr/bin/env python3
"""02_gpu_power_monitor.py

Collect GPU power metrics via `nvidia-smi` at a fixed interval.

Example:
  python3 02_gpu_power_monitor.py --interval 0.5 --out gpu_power.csv

CSV Columns:
  ts_epoch_ms,gpu_index,power_draw_w,power_limit_w,temp_gpu_c

Notes:
- GPU statistics are host-level/aggregated per GPU.
- Use timestamps to align with container CPU + log streams.
"""
...
def main() -> int:
    ...
    out_f.write("ts_epoch_ms,gpu_index,power_draw_w,power_limit_w,temp_gpu_c\n")
        ...
        while True:
            ...
            try:
                for row in run_nvidia_smi_query():
                    ...
                    if len(parts) < 4:
                        continue
                    gpu_index, pwr_draw, pwr_limit, temp_c = parts[0], parts[1], parts[2], parts[3]
                    out_f.write(f"{ts},{gpu_index},{pwr_draw},{pwr_limit},{temp_c}\n")
