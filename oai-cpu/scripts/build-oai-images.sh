...
# functions
function usage() {
    echo "Usage: $0 [-h|--help] [--cpu|--cuda] [--debug] [--tag <tagname>] [--no-cache] [--ci] [--force-platform <platform>] <openairinterface5g_dir>"
    exit 1
}
...
function build_cpu_images() {
    echo "$platform" > ${oai_path}/host_product_family
    echo "Building CPU-only (non-CUDA) OAI images"
    ...
    log_check_status docker build --progress plain \
        $cache_opts --target ran-base --tag ran-base:latest \
        --file docker/Dockerfile.base.ubuntu .

    log_separator
    log "CPU build image"
    log_check_status docker build --progress plain \
        $cache_opts --target ran-build --tag ran-build:latest \
        --file docker/Dockerfile.build.ubuntu .

    log_separator
    log "CPU gNB (CU/DU runtime image)"
    log_check_status docker build --progress plain \
        $cache_opts --target oai-gnb --tag oai-gnb:${tag} \
        --file docker/Dockerfile.gNB.ubuntu .

    log_separator
    log "CPU nrUE runtime image"
    log_check_status docker build --progress plain \
        $cache_opts --target oai-nr-ue --tag oai-nr-ue:${tag} \
        --file docker/Dockerfile.nrUE.ubuntu .

    log_separator
    log "FlexRIC"
    log_check_status docker build \
        $cache_opts --target oai-flexric-fixed --tag oai-flexric:${tag} \
