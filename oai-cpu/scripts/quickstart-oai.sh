...
# functions
function usage() {
    echo "Usage: $0 [-h|--help] [--clean] [--debug] [--no-build] [--tag <tagname>] [--ci] [--oai-version <oai-version>] [--no-patch] [--cpu|--cuda] --source <kit-rootdir> --dest <openairinterface5g_dir>"
    exit 1
}
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --debug) debug=1 ;;
        --ci) ci=1 ;;
        --no-patch|--no-srk-patch) apply_patches=0 ;;
        --cpu|--no-cuda) build_mode="cpu" ;;
        --cuda) build_mode="cuda" ;;
        *) usage; exit 1 ;;
    esac
    shift
done
...
if [ "$apply_patches" = "1" ]; then
   ...
   echo "Skipping SRK patches (--no-patch)."
fi
...
if [ "$no_build" = "0" ]; then
    ...
    "${source_dir}/scripts/build-oai-images.sh" $extra_opts --tag "$TAG" --${build_mode} "$dest_dir"
fi
