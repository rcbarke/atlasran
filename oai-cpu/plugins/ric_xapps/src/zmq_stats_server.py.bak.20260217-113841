...
class FlexRICStatsPublisher:
        ...
        try:
            ...
            if self.publish_count % 20 == 0:
                ...
                for ue in stats_data.get('UE_stats', []):
                    ...
                    logger.info(
                        f"  {ue['ue_id']} {imsi_str}RNTI:{ue['rnti']} "
                        f"MCS↓:{ue['mcs_down']}/↑:{ue['mcs_up']} "
                        f"BLER↓:{ue['bler_down']:.3f}/↑:{ue['bler_up']:.3f} "
                        f"PRBs(inst)↓:{ue.get('prbs_inst_down', 0)}/↑:{ue.get('prbs_inst_up', 0)} "
                        f"PRBs(max)↓:{ue['num_prbs_down']}/↑:{ue['num_prbs_up']} "
                        f"PRBs(delta)↓:{ue['delta_prbs_down']}/↑:{ue['delta_prbs_up']} "
                        f"PRBs(cum)↓:{ue['aggr_prbs_down']}/↑:{ue['aggr_prbs_up']}"
                    )
    ...
    def _init_flexric(self):
        ...
        try:
            ...
            du_node = select_single_mac_node_by_nb_id(self.nodes)
            if du_node is None:
                logger.error("✗ No E2 nodes found to select for MAC.")
                return False
            ...
            try:
                ...
                pass
            logger.info(f"✓ Selected MAC node by nb_id (DU candidate): {str(du_node.id)} nb_id={nb_id_val}")

            mac_nodes = [du_node]
            if len(mac_nodes) == 0:
                logger.error("✗ No MAC-capable nodes found (expected DU or monolithic gNB).")
                return False

            for i, node in enumerate(mac_nodes):
                try:
                   plmn = f"{node.id.plmn.mcc}.{node.id.plmn.mnc}"
                except Exception:
                   plmn = "<unknown-plmn>"
                logger.info(f"MAC target node {i + 1}: PLMN {plmn} node={str(node.id)}")
                
            logger.info("Subscribing to MAC statistics...")

            for node in mac_nodes:
                mac_cb = MACStatsCallback()
                self.mac_callbacks.append(mac_cb)  # keep callback alive
                try:
                    handler = ric.report_mac_sm(node.id, ric.Interval_ms_1, mac_cb)
                    self.mac_handlers.append(handler)
                    logger.info(f"✓ MAC subscription active on node: {node.id}")
                except Exception as e:
                    logger.exception(f"MAC subscription FAILED on node {node.id}: {e}")
                    continue

            if len(self.mac_handlers) == 0:
                logger.error("✗ No MAC subscriptions are active after attempting all nodes.")
                return False
    ...
    def handle(self, ind):
        ...
        if hasattr(ind, 'ue_stats') and len(ind.ue_stats) > 0:
            ...
            for ue_idx in range(num_ues):
                ...
                # Instantaneous PRBs from this MAC indication (current scheduling RBs)
                ue_data["prbs_inst_down"] = ue.dl_sched_rb if hasattr(ue, "dl_sched_rb") else 0
                ue_data["prbs_inst_up"]   = ue.ul_sched_rb if hasattr(ue, "ul_sched_rb") else 0
