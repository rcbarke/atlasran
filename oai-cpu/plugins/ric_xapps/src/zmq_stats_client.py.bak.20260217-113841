...
class UEStatsSubscriber:
    ...
    def _print_ue_details(self, ue_stats: Dict[str, Any]):
        ...
        for ue in ue_stats.get("UE_stats", []):
            ...
            print(
                f"{ue.get('ue_id')}, {imsi_str}RNTI {rnti} (0x{rnti:04x}): "
                f"MCS ↑{ue.get('mcs_up')}/↓{ue.get('mcs_down')}, "
                f"BLER ↑{ue.get('bler_up', 0):.3f}/↓{ue.get('bler_down', 0):.3f}, "
                f"PRBs(inst) ↑{ue.get('prbs_inst_up', 0)}/↓{ue.get('prbs_inst_down', 0)}, "
                f"PRBs(max) ↑{ue.get('num_prbs_up', 0)}/↓{ue.get('num_prbs_down', 0)}, "
                f"PRBs(delta) ↑{ue.get('delta_prbs_up', 0)}/↓{ue.get('delta_prbs_down', 0)}, "
                f"PRBs(cum) ↑{ue.get('aggr_prbs_up', 0)}/↓{ue.get('aggr_prbs_down', 0)}"
            )
