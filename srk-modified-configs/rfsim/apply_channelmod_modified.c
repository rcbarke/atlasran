...
void rxAddInput( const c16_t *input_sig,
  ...
  // 1. Convert noise power from dB to linear scale
  const double linear_noise_power = pow(10.0, channelDesc->noise_power_dB / 10.0);

  // 2. Calculate the correct standard deviation for I and Q components
  //    P_noise = 2 * sigma^2  =>  sigma = sqrt(P_noise / 2)
  const double noise_std_dev = sqrt(linear_noise_power / 2.0);
  ...
  for (int i=0; i<nbSamples; i++) {
    ...
    out_ptr->r += lround(rx_tmp.r*pathLossLinear + (noise_std_dev*gaussZiggurat(0.0,1.0)) * 1000);
    out_ptr->i += lround(rx_tmp.i*pathLossLinear + (noise_std_dev*gaussZiggurat(0.0,1.0)) * 1000);
    out_ptr++;
  }
