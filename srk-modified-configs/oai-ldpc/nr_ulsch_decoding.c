...
static inline uint64_t oai_now_ns(void)
{
  struct timespec ts;
                      ...
                      int nb_pusch)
{
  ...
  // --- LDPC timing (CPU baseline) ---
  static _Atomic uint64_t acc_total_ns = 0;
  static _Atomic uint64_t acc_segs     = 0;
  static _Atomic uint64_t acc_calls    = 0;
  static _Atomic uint64_t last_log_ns  = 0;

  uint32_t C_total = 0;
  for (uint8_t pusch_id = 0; pusch_id < nb_pusch; pusch_id++) {
    C_total += TBs[pusch_id].C;
  }

  // Time just the LDPC decode invocation
  uint64_t t0 = oai_now_ns();

  int ret_decoder = phy_vars_gNB->nrLDPC_coding_interface.nrLDPC_coding_decoder(&slot_parameters);

  // post decode
  uint64_t t1 = oai_now_ns();
  uint64_t dt = t1 - t0;
  if (C_total > 0) {
    atomic_fetch_add_explicit(&acc_total_ns, dt, memory_order_relaxed);
    atomic_fetch_add_explicit(&acc_segs, C_total, memory_order_relaxed);
    atomic_fetch_add_explicit(&acc_calls, 1, memory_order_relaxed);
  }

  uint64_t prev = atomic_load_explicit(&last_log_ns, memory_order_relaxed);
  if (prev == 0) {
    atomic_compare_exchange_strong_explicit(&last_log_ns, &prev, t1,
                                         memory_order_relaxed, memory_order_relaxed);
    prev = atomic_load_explicit(&last_log_ns, memory_order_relaxed);
  }

  if (t1 - prev >= 1000000000ULL) {
    // try to become the printer thread
    uint64_t expected = prev;
    if (atomic_compare_exchange_strong_explicit(&last_log_ns, &expected, t1,
                                             memory_order_relaxed, memory_order_relaxed)) {
      uint64_t tot = atomic_exchange_explicit(&acc_total_ns, 0, memory_order_relaxed);
      uint64_t seg = atomic_exchange_explicit(&acc_segs, 0, memory_order_relaxed);
      uint64_t cal = atomic_exchange_explicit(&acc_calls, 0, memory_order_relaxed);

      double total_us = cal ? ((double)tot / 1000.0) / (double)cal : 0.0;
      double seg_us   = seg ? ((double)tot / 1000.0) / (double)seg : 0.0;

      LOG_I(NR_PHY, "CPU LDPC decoder: %8.2f us (%8.2f us / seg)\n", total_us, seg_us);
    }
  }
